...
const DynamicField = memo(
  ({ field, formik, dispatch, setFileAndItsFilePath }) => {
    ...
    const handleFileUpload = useCallback(
      (e) => {
        ...
        if (field.accept) {
          ...
          if (!matches) {
            formik.setFieldError(
              field.id,
              `File type not allowed (${field.accept})`
            );
            return;
          }
        }
        ...
        if (file.size > maxBytes) {
          formik.setFieldError(
            field.id,
            `File too large. Max ${(maxBytes / 1024 / 1024).toFixed(
              1
            )} MB allowed.`
          );
          return;
    ...
    const commonProps = useMemo(
      () => ({
        ...
        value:
          formik.values[field.id] === undefined ||
          formik.values[field.id] === null
              ...
              <Select
                ...
                renderValue={(selected) =>
                  selected ? (
                    ...
                    <span style={{ color: "#9e9e9e" }}>Select an option</span>
                  )
                }
              >
                ...
                <FormControlLabel
                  ...
                  control={
                    <Radio
                      sx={{
                        color: "#9e9e9e",
                        "&.Mui-checked": { color: CONFIG.COLORS.PRIMARY },
                      }}
                    />
                  }
                  label={opt}
                ...
                <FormControlLabel
                  ...
                  control={
                    <Checkbox
                      ...
                      checkedIcon={
                        <CheckBox sx={{ color: CONFIG.COLORS.PRIMARY }} />
                      }
                    />
                  }
                  label={opt}
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  onChange={(e) =>
                    formik.setFieldValue(field.id, e.target.checked)
                  }
                  sx={{
                    color: "#9e9e9e",
                    "&.Mui-checked": { color: CONFIG.COLORS.PRIMARY },
          ...
          <FieldWrapper field={field} formik={formik}>
            <Box sx={{ mt: 1 }}>
              ...
              <Button
                variant="outlined"
                component="label"
                startIcon={UploadIcon ? <UploadIcon /> : null}
                sx={{
                  borderColor: "#b39ddb",
  ...
  (fields || []).forEach((f) => {
    ...
    switch (f.type) {
      ...
      case "number":
        ...
        if (f.min)
          rule = rule.min(+f.min, `${f.label} must be at least ${f.min}`);
        if (f.max)
          rule = rule.max(+f.max, `${f.label} must be at most ${f.max}`);
        break;
      ...
      case "uploadFile":
        const maxB =
          (f.maxSize && parseInt(f.maxSize, 10) < 100
            ? parseInt(f.maxSize, 10) * 1024 * 1024
            : parseInt(f.maxSize, 10)) || CONFIG.MAX_FILE_MB * 1024 * 1024;
        rule = Yup.mixed().test(
          "fileSize",
          `File too large (max ${Math.round(maxB / 1024 / 1024)}MB)`,
          (value) => {
    ...
    if (f.required) {
      if (f.type === "checkbox")
        rule = rule.oneOf([true], `${f.label} is required`);
...
export default function FormPreview({ previewData }) {
  ...
  const schema = useMemo(() => {
    if (previewData) return previewData; // directly use the passed preview schema
    if (formData?.schema) return formData.schema; // fallback to fetched form
    return null;
  ...
  const initialValues = useMemo(() => {
    ...
    return schema.fields.reduce((acc, field) => {
      if (field.type === "checkbox")
        acc[field.id] = !!field.defaultChecked || false;
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setMountedFields((prev) =>
        prev.map((m) =>
          m.state === "entering" ? { ...m, state: "present" } : m
        )
      );
    }, 20);
  ...
  useIdlePrefetch(
    bannerFile ? `${BACKEND_URL}${FILE_PATH}${bannerFile}` : null
  );
    ...
    return (
      <Box
        sx={{
          ...
          backgroundColor: "#fff",
          minHeight: 80,
          height: { xs: 80, sm: 120, md: 160, lg: 200 },
          maxWidth: 1600,
          width: "100%",
        }}
      >
        <picture>
          <source
            srcSet={`${BACKEND_URL}${FILE_PATH}${bannerFile}`}
            type="image/webp"
          />
          <Box
            component="img"
                  ...
                  <Box sx={{ flex: 1 }}>
                    {otpVerified ? (
                      ...
                      <>
                        ...
                        >
                          We will send an OTP to your email address to verify
                          your identity.
                        </Typography>
                      </>
                    )}
