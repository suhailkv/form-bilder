...
//   setUploadError, fetchForm, createForm, setEmailVerification,
...
//            Email Required <Switch checked={schema.emailVerification ?? true} onChange={(e) =>
//              dispatch(setEmailVerification(e.target.checked))} />
//           </Typography>
...
//   setFocusedQuestion, setMenuAnchor, setMenuQuestionId,
//   setMobileMenuOpen, setConditionalLogicOpen, setDraggedId,
//   setUploadError, fetchForm, createForm, setEmailVerification, uploadFile
// } from "../redux/features/formCreationSlice";
...
//       const storedName = await dispatch(uploadFile(file)).unwrap();
      
//       // Validate the returned storedName
//       if (!storedName) {
//         throw new Error("Invalid storedName returned from server");
//       }

//       // Construct the full URL to display the image
//       // Adjust this path based on how your server serves static files.
//       // If files are stored in 'uploads' folder and served via /uploads/, use the line below.
//       // If served from root or another path, update accordingly (e.g., `http://172.16.3.209:5000/static/${storedName}`).
//       const imageUrl = `http://172.16.3.209:5000/uploads/${storedName}`;
      
//       console.log("Uploaded image URL:", imageUrl); // For debugging

//       handleBannerImageChange(imageUrl);
//     } catch (error) {
//       console.error("Banner upload failed:", error); // For debugging
//       dispatch(setUploadError(error.message || "Failed to upload the image. Please check server logs."));
//       // Optionally reset the input file value to allow re-upload
//       event.target.value = '';
//     }
//   };
...
//                   style={{ width: "100%", height: "100%", objectFit: "contain" }}
//                   onError={(e) => {
//                     console.error("Image load error for URL:", schema.bannerImage); // For debugging
//                     // Provide more specific guidance in the error message
//                     dispatch(setUploadError(`Failed to load the image from "${schema.bannerImage}". Ensure your backend serves static files from the 'uploads' directory using something like: app.use('/uploads', express.static('uploads'));`));
//                     handleBannerImageChange(null);
//                   }}
//                   onLoad={() => {
//                     console.log("Image loaded successfully:", schema.bannerImage); // For debugging
//                     dispatch(setUploadError(null)); // Clear any previous errors on successful load
//                   }}
//                 />
//                 <Button
//                   variant="contained"
...
import {
  ...
  setMobileMenuOpen, setConditionalLogicOpen, setDraggedId,
  setUploadError, fetchForm, createForm, setEmailVerification,
  uploadBannerImage, // Import the new thunk
} from "../redux/features/formCreationSlice";
...
export default function QuestionBuilder() {
  ...
  // Redux state (includes new banner states)
  const {
    schema,
    undoStack,
    redoStack,
    ...
    uploadError,
    loading,
    loadingBannerImage, // New
    bannerImageError, // New
  } = useSelector((state) => state.formCreation);
  ...
  // Handlers (your existing ones – adding banner upload)
  const handleSchemaChange = (newFields, newThankYouMessage) => {
    dispatch(setUndoStack([...undoStack, deepClone(schema)]));
    dispatch(setRedoStack([]));
    dispatch(
  ...
  // Banner Image Upload Handler (dispatches uploadBannerImage)
  const handleBannerImageUpload = (event) => {
    const file = event.target.files[0];
    if (!file) return;
    ...
    // Dispatch the upload thunk (handles upload + fetch chain)
    dispatch(uploadBannerImage(file));
  };

  const handleRemoveBanner = () => {
    dispatch(setSchema({ ...schema, bannerImage: null }));
    dispatch(setUploadError(null));
    dispatch({ type: 'formCreation/clearBannerImage' }); // Optional: Add a reducer to clear if needed
  };
  ...
  return (
    <Box sx={{ flexGrow: 1, minHeight: "100vh", backgroundColor: "#efebf9" }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: { xs: 1, sm: 1.5, md: 2 } }}>
            ...
            <Tooltip title="Save Form" arrow>
              <IconButton
                ...
                onClick={handleSaveForm}
              >
                <Icon icon="mdi:content-save-check-outline" style={{ color: "#4e6ca7ff", fontSize: 30 }} />
              </IconButton>
            </Tooltip>
          </Box>
          <Box sx={{ display: "flex", alignItems: "center", gap: { xs: 0.5, sm: 1, md: 1.5 } }}>
      ...
      <Box sx={containerStyles}>
        ...
        <Paper sx={headerStyles}>
          <Box sx={{ mb: isXs ? 2 : 3, position: "relative" }}>
            {schema.bannerImage ? (
              ...
              >
                <img
                  src={schema.bannerImage} // Base64 data URL
                  alt="Banner"
                  style={{ width: "100%", height: "100%", objectFit: "contain" }}
                  onError={() => {
                    dispatch(setUploadError("Failed to load the image."));
            ...
            {(uploadError || bannerImageError) && (
              <Typography
                color="error"
                sx={{ fontSize: isXs ? "0.8rem" : "0.9rem", mt: 1, textAlign: "center" }}
              >
                {uploadError || bannerImageError}
              </Typography>
            )}
        ...
        {/* Questions Rendering (unchanged from your code) */}
        {schema.fields.map((q) => {
          const isFocused = focusedQuestion === q.id;
          const isConditionalOpen = conditionalLogicOpen === q.id;
          const isDragging = draggedId === q.id;
